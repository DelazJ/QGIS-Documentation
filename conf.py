...
extlinks = {  # api website: docs master branch points to '/' while x.y points to x.y
    "api": (
        "https://api.qgis.org/api/{}%s".format(
            "".join([version, "/"]) if version != "testing" else ""
        ),
        None,
    ),
...
latex_documents = [
    (
        "docs/about/index",
        "AboutQGIS.tex",
        f"About QGIS {version}",
        "QGIS Project",
        "manual",
    ),
    (
        "docs/user_manual/index",
        "DesktopUserGuide.tex",
...
class BetterDocTestBuilder(ext_doctest.DocTestBuilder):
    ...
    def skipped(self, node) -> bool:
        ...
        if not to_skip and os.environ.get("SINGLE_TEST") is not None:
            to_skip = os.path.basename(node.source or "") != os.environ.get(
                "SINGLE_TEST"
            )
