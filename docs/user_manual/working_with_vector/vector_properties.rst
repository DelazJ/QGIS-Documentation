...
* Depending on the data provider, a :guilabel:`Layer source` group indicates the path
  ...
  * When the layer is stored as file on disk, edit the path shown in the text box
    ...
    Both layers do not need to share attribute fields, geometry type or file formats.
  * When the layer is provided by an ArcGIS Feature service,
...
display when creating new features, editing or querying existing one.
This affects them in both :ref:`table and form views <attribute_table_view>`.
You can define:

* the look and the behavior (label, visibility, widget, constraints...) of each field
  in the current layer, the :ref:`joined layers <sec_joins>`
  and the :ref:`relational ones <vector_relations>`;
* the placement of enabled :ref:`actions <actions_menu>`;
* the form's structure (custom or autogenerated);
* extra HTML, QML, text widgets to enhance the form;
...
* ``Drag-and-drop designer``: other than widget customization, the form
  structure can be made more complex eg, with widgets embedded in groups and tabs.
...
.. figure:: img/attributes_form_properties.png
   :align: center
   :width: 70%

   Attributes Form properties dialog
...
displays lists of fields (from the current layer, the join and relation layers)
and the enabled actions that would be shown in the form.
Select a field and you can configure its appearance and behavior in the right panel:
...
In this editor mode, only the properties can be viewed for the actions.
Moreover, they will be displayed through an :guilabel:`Actions` drop-down menu in the feature form,
and following the :guilabel:`Show in attribute table` property in other contexts.
